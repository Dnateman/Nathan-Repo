...
function make_fields_read_only(frm) {
    var read_only_fields = [
        'transaction_type', 'recurring_entry', 'amount', 'effective_from_date', 'effective_to_date', 'remarks', 'bank_name',
        'bank_type', 'bank_account', 'account_type', 'branch_code', 'total_yr_days', 'rate_type',
        'rate', 'job_grade', 'payroll_schedule', 'rate_class', 'period_group', 'min_take_home', 'cost_center', 'mode_of_payment',
        'mwe_loc', 'payroll_group', 'mth_percentage',
        'sss_no', 'sss_mode', 'hdmf_no', 'hdmf_mode', 'phic_no', 'phic_mode', 'tin', 'rdo_code', 'sss_manual',
        'hdmf_manual', 'whtax_mode', 'phic_manual', 'whtax_manual', 'sss_freq', 'hdmf_freq', 'phic_freq', 'whtax_freq', 'part_time',
    ];
    ...
    if (frappe.user.has_role('HR1')) {
        ...
        set_fields_read_only(rate_table_fields);
        set_fields_read_only(banks_table_fields);
    } else if (frappe.user.has_role('HR2 Payroll Processor')) {
        set_fields_read_only(payroll_processor_fields);
        set_fields_read_only(family_members_fields);
        set_fields_read_only(education_fields);
        set_fields_read_only(license_records_fields);
        set_fields_read_only(work_history_fields);
        set_fields_read_only(trainings_seminar_fields);
        set_fields_read_only(approvers_fields);
        ...
        var all_fields = frm.fields_dict;
        for (var fieldname in all_fields) {
            if (all_fields.hasOwnProperty(fieldname)) {
                set_fields_read_only(fieldname);
            }
        }
    }
}
