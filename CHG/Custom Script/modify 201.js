...
frappe.ui.form.on("Employee", {
    ...
    async after_save(frm){
        ...
        let data = await frappe.db.get_list("Employee ID Monitoring", { filters: filters, fields: ["*"], limit: 1500000 });
        console.log('data',data)
        if(data == undefined && !/New Employee/.test(frm.doc.name) ){
            post_in_monitoring(frm);
        }else{
            console.log('old')
        }
        autoEmpIDinUser(frm);
    ...
    classification: async function(frm) {
        ...
        if(frm.doc.date_hired !== undefined){
            automateEmployeeID(frm)
        }else{frappe.msgprint(
                "Please insert<strong> Date Hired </strong>first."
            );
            frappe.validated = false;}
    },
    
    async before_save(frm){
	if(frm.doc.__islocal == 1){blockExisting(frm)}
   ...
   async branch(frm){
        //const empRole = await frappe.db.get_value('Branch', { 'name': frm.doc.branch }, 'cost_center'); 
...
async function blockExisting(frm){
    const empName = await frappe.db.get_value('Employee ID Monitoring', { 'name': frm.doc.employee_id }, 'name'); 
    const empBioID = await frappe.db.get_value('Employee ID Monitoring', { 'name': frm.doc.employee_id }, 'biometrics_id'); 
    if (empName.message && empName.message.name) {
        frappe.msgprint(
            "<strong>Employee ID is already used</strong>."
        );
        frappe.validated = false;
        return;
    } else {
    ...
    if (empBioID.message.biometrics_id == frm.doc.biometrics_id){
        frappe.msgprint(
            "<strong>Biometrics ID is already used</strong>."
        );
        frappe.validated = false;
        return;
    }
...
async function RecordTo201(frm) {
    ...
    let parent_obj = {
                ...
                'biometrics_id': frm.doc.biometrics_id,
...
async function post_in_monitoring(frm){
    ...
    if(data !== undefined){
        ...
        bulk_update_monitoring(frm);
    }
    else{
        RecordTo201(frm);
    }
}
async function bulk_update_monitoring(frm) {
    // Step 1: Retrieve all records matching the filter
    let filters = { 'employee_id': frm.doc.name }; // Assuming this filter criteria is needed
    let fields = ['employee_id', 'biometrics_id', 'company', 'date_hired', 'classification', 'is_active'];

    let results = await frappe.db.get_list("Employee ID Monitoring", { 
        filters: filters, 
        fields: fields, 
        limit: 1000000
    });
    ...
    if (results.length > 0) {
        ...
        let updatePromises = results.map(record => {
            return frappe.db.set_value("Employee ID Monitoring", record.employee_id, {
                'biometrics_id': frm.doc.biometrics_id,  // Example: Update with new biometrics_id from form
                'company': frm.doc.company,              // Example: New company value from form
                'date_hired': frm.doc.date_hired,        // Example: New date hired from form
                'classification': frm.doc.classification, // Example: New classification from form
                'is_active': frm.doc.is_active           // Example: New active status from form
            });
        });
...
async function automateEmployeeID(frm){
    let series_id = "";
        let class_list = [];
        let current_count = 0;
...
async function get_UserDet(frm,name){
    ...
    let data = frappe.db.get_list("User",{filters: _filters, fields: _fields, limit: 100});
...
/*
        ...
        TC-CS-ER-003                Nathan                          2024-09-10                      Added Automation of username as Employee_ID
*/
//------------------------------------------------------------------------------------------------------------------------------------------------